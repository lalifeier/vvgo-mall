...
func (c *Client) Intercept(interceptors ...Interceptor) {
	c.AccountUser.Intercept(interceptors...)
}

// Mutate implements the ent.Mutator interface.
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	switch m := m.(type) {
	case *AccountUserMutation:
		return c.AccountUser.mutate(ctx, m)
	default:
...
// A call to `Intercept(f, g, h)` equals to `accountuser.Intercept(f(g(h())))`.
func (c *AccountUserClient) Intercept(interceptors ...Interceptor) {
	c.inters.AccountUser = append(c.inters.AccountUser, interceptors...)
}
...
func (c *AccountUserClient) Query() *AccountUserQuery {
	return &AccountUserQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeAccountUser},
		inters: c.Interceptors(),
	}
}

// Get returns a AccountUser entity by its id.
func (c *AccountUserClient) Get(ctx context.Context, id uint32) (*AccountUser, error) {
	return c.Query().Where(accountuser.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *AccountUserClient) GetX(ctx context.Context, id uint32) *AccountUser {
...
func (c *AccountUserClient) Interceptors() []Interceptor {
	return c.inters.AccountUser
}

func (c *AccountUserClient) mutate(ctx context.Context, m *AccountUserMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&AccountUserCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&AccountUserUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&AccountUserUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&AccountUserDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown AccountUser mutation op: %q", m.Op())
	}
}
