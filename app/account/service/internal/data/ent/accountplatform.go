...
type AccountPlatform struct {
	...
	// PlatformToken holds the value of the "platform_token" field.
	// 平台access_token
...
func (*AccountPlatform) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		switch columns[i] {
		case accountplatform.FieldID, accountplatform.FieldUID, accountplatform.FieldType, accountplatform.FieldCreateAt, accountplatform.FieldUpdateAt:
	...
	for i := range columns {
		...
		case accountplatform.FieldCreateAt:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				ap.CreateAt = int32(value.Int64)
			}
		case accountplatform.FieldUpdateAt:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field update_at", values[i])
			} else if value.Valid {
				ap.UpdateAt = int32(value.Int64)
			}
		}
	}
	return nil
...
func (ap *AccountPlatform) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", ap.CreateAt))
	builder.WriteString(", update_at=")
	builder.WriteString(fmt.Sprintf("%v", ap.UpdateAt))
	builder.WriteByte(')')
