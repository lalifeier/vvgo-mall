...
func (o *option) Parse(url string) (*pb.PaserReply, error) {

	douyin := &DouyinData{}

	client := resty.New()
	resp, err := client.R().SetResult(douyin).EnableTrace().Get(url)
	if err != nil {
		return nil, err
	}
	...
	realURL := douyin.ItemList[0].Video.PlayAddr.URLList[0]
	...
	streams := map[string]*pb.Stream{
		"default": {
			Parts: []*pb.Part{part},
			Size:  0,
		},
	}
	return &pb.PaserReply{
		Site:    "抖音 douyin.com",
