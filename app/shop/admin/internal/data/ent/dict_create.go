...
func (dc *DictCreate) SetNillableDictTypeID(i *int64) *DictCreate {
	if i != nil {
		dc.SetDictTypeID(*i)
	}
	return dc
...
func (dc *DictCreate) defaults() error {
	...
	if _, ok := dc.mutation.DictTypeID(); !ok {
		v := dict.DefaultDictTypeID
		dc.mutation.SetDictTypeID(v)
	}
...
func (dc *DictCreate) check() error {
	...
	if _, ok := dc.mutation.DictTypeID(); !ok {
		return &ValidationError{Name: "dict_type_id", err: errors.New(`ent: missing required field "Dict.dict_type_id"`)}
	}
	if _, ok := dc.mutation.Label(); !ok {
